...
void OpenChallenge::loop() {
    ...
    switch (_state) {
        ...
        case FORWARD:
            ...
            if(_turnCount==0) _enter(CHECK_TURN);
        ...
        case CHECK_TURN:
            if (_canTurnLeft()) { _beginTurn(-90.0f);
            turn_right = false;}
            else if (_canTurnRight()) {_beginTurn(+90.0f);
            turn_right = true;}
            else                      _enter(FORWARD);
            break;

        case TURNING:
            if (_turnStep()) _enter(FORWARD);
            break;      
...
void OpenChallenge::_getAwayWalls() {
    ...
    const float k = 1.7f;
    if (right <= 30 && right != 0) {
        _steeringCorr += k * (30 - right);
    }
    if (left <= 30 && left != 0) {
        _steeringCorr -= k * (30 - left);
    }
}
...
bool OpenChallenge::_shouldConsiderTurn()  {
    if ((millis() - _lastTurnMs) < TURN_COOLDOWN_MS) return false;
