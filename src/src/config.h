...
const float STEERING_KP = 2.9;     // Proportional gain for steering correction. Tune this!

// -- Sensor Settings --
const int ULTRASONIC_MAX_DISTANCE_CM = 200; // Max distance for ultrasonic sensors.

// -- Navigation Logic --
const float TURN_DECISION_DISTANCE_CM = 70.0; // How close to a wall before deciding to turn.
const float TURN_CLEARANCE_DISTANCE_CM = 120.0; // How much space must be clear to the side to make a turn.
const unsigned long TURN_COOLDOWN_MS = 2500;  // Minimum time between turns.
